...
export async function generateSoundEffectWithRunway(
  promptText: string,
  options?: {
    duration?: number // 0.5 to 30 seconds
    loop?: boolean
  }
): Promise<string> {
  const body: any = {
  ...
  console.log("🔊 RunwayML sound-effect — creating task…")
  const createRes = await runwayFetch("/sound_effect", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML sound effect error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML sound task created: ${taskId}`)
  ...
  throw new Error("RunwayML returned no output audio")
}
 ...
 * @returns URL of the generated audio
 */
export async function generateTextToSpeechWithRunway(
  promptText: string,
  voice: RunwayVoicePreset,
  options?: {
  ...
  console.log(`🗣️ RunwayML text-to-speech (${voice}) — creating task…`)
  const createRes = await runwayFetch("/text_to_speech", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML text-to-speech error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML TTS task created: ${taskId}`)
...
/**
 * Transform an existing audio file's voice to another voice.
 * @param mediaUri URL or Data URI of the audio
 * @param voice Target voice preset
 */
export async function generateSpeechToSpeechWithRunway(
  mediaUri: string,
  voice: RunwayVoicePreset,
  options?: {
    model?: string // e.g., "eleven_multilingual_sts_v2"
  }
): Promise<string> {
  const body: any = {
  ...
  console.log(`🗣️ RunwayML speech-to-speech (${voice}) — creating task…`)
  const createRes = await runwayFetch("/speech_to_speech", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML speech-to-speech error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML STS task created: ${taskId}`)
 ...
 * @param audioUri URL or Data URI of the source audio
 * @param targetLang The language to dub to (e.g. "es", "fr", "de")
 */
export async function generateVoiceDubbingWithRunway(
  audioUri: string,
  targetLang: string,
  options?: {
    model?: string // default "eleven_voice_dubbing"
  }
): Promise<string> {
  const body: any = {
    model: options?.model || "eleven_voice_dubbing",
    audioUri,
    targetLang,
  }

  console.log(`🌍 RunwayML voice-dubbing (${targetLang}) — creating task…`)
  const createRes = await runwayFetch("/voice_dubbing", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML voice-dubbing error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML dubbing task created: ${taskId}`)
 ...
 * @param audioUri URL or Data URI of the source audio
 */
export async function generateVoiceIsolationWithRunway(
  audioUri: string
): Promise<string> {
  const body: any = {
  ...
  console.log("🎙️ RunwayML voice-isolation — creating task…")
  const createRes = await runwayFetch("/voice_isolation", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML voice-isolation error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML voice isolation task created: ${taskId}`)
...
/**
 * Animate a character portrait from audio or a video reference.
 * @param character URL or Data URI of the character image
 * @param reference URL or Data URI of the driving audio or video
 */
export async function generateCharacterPerformanceWithRunway(
  character: string,
  reference: string,
  options?: {
    model?: string // default "act_two"
  }
): Promise<string> {
  const body: any = {
    model: options?.model || "act_two",
    character,
    reference,
  }

  console.log("🎭 RunwayML character-performance — creating task…")
  const createRes = await runwayFetch("/character_performance", {
    method: "POST",
    body: JSON.stringify(body),
  })
  ...
  if (!createRes.ok) {
    const errData = await createRes.json().catch(() => ({}))
    throw new Error(`RunwayML character-performance error: ${createRes.status} — ${JSON.stringify(errData)}`)
  }

  const { id: taskId } = await createRes.json()
  console.log(`✅ RunwayML character performance task created: ${taskId}`)
