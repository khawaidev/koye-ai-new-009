 ...
 *
 * An API key's model is "skipped for 1 minute" when its RPM or TPM is exceeded.
 * A key is "retired for 1 day" only when RPD is exhausted on ALL its models.
 *
 * If ALL Gemini keys × models are exhausted → throws ALL_GEMINI_KEYS_EXHAUSTED
 * so the caller can surface a Gemini exhaustion fallback.
 */
...
export const MODEL_SPECS: ModelSpec[] = [
  ...
  { id: "gemma-4-26b",                   rpm: 5,  tpm: 250_000, rpd: 20 },
  { id: "gemma-3-27b",                   rpm: 5,  tpm: 250_000, rpd: 20 },
]

function normalizeRouteOptions(options?: string | SmartRouteOptions): SmartRouteOptions {
  if (!options) return {}
...
async function getAllUsageForKey(apiKeyHash: string): Promise<Map<string, UsageRecord>> {
  ...
  return new Promise((resolve) => {
    const tx = db.transaction(STORE_NAME, "readonly")
    const store = tx.objectStore(STORE_NAME)
    // Read all model records in parallel within the same transaction
...
export async function pickRoute(
  ...
  for (const apiKey of GEMINI_API_KEYS) {
    ...
    if (preferredModelId) {
      ...
      if (prefIdx > -1) {
        ...
        modelsToTry.unshift(pref)
      }
    }

    for (const spec of modelsToTry) {
      const rec = usageMap.get(spec.id)!
      const available =
      ...
      if (available) {
        if (preferredModelId && spec.id !== preferredModelId) {
          console.log(`[SmartRouter] Preferred model ${preferredModelId} not available, falling back to ${spec.id}`)
        }
  ...
  // If we reach here, we found no available models
  // Let's determine if it's because ALL keys are day exhausted, or just minute exhausted
  let allDayExhausted = true
  for (const apiKey of GEMINI_API_KEYS) {
    const kh = hashKey(apiKey)
    const usageMap = await getAllUsageForKey(kh)
    const keyAllDayExhausted = MODEL_SPECS.every(spec => {
      const rec = usageMap.get(spec.id)!
      return rec.rpd_used >= spec.rpd
    })
    if (!keyAllDayExhausted) {
      allDayExhausted = false
      break
    }
  }

  if (allDayExhausted) {
    throw new Error("ALL_GEMINI_KEYS_DAY_EXHAUSTED")
  } else {
    throw new Error("ALL_GEMINI_KEYS_MINUTE_EXHAUSTED")
  }
}
...
export async function withSmartRoute<T>(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      route = await pickRoute(estimatedTokens, options)
    } catch (e) {
    ...
    try {
      ...
      // Non-rate-limit error (e.g. 404) → don't retry, propagate
      throw error
    }
  }
