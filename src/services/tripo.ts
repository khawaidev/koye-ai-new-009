...
export async function createPreRigCheck(options: { original_model_task_id: string }): Promise<any> {
    ...
    for (let i = 0; i < keys.length; i++) {
        const apiKey = keys[i];
        try {
            console.log(`[Tripo] Creating pre-rig check task with key ${i + 1}`);
...
export async function createRiggingTask(options: { 
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        try {
            ...
            if (options.spec) payload.spec = options.spec;
...
async function triggerRetarget(
    rigTaskId: string, 
    animation?: string, 
    animations?: string[],
    options?: { bake_animation?: boolean; animate_in_place?: boolean }
): Promise<string> {
    const keys = getAllTripoApiKeys();
    for (let i = 0; i < keys.length; i++) {
        try {
            ...
            if (animations && animations.length > 0) {
                ...
                throw new Error("Must provide animation or animations array");
            }

            if (options?.bake_animation !== undefined) payload.bake_animation = options.bake_animation;
            if (options?.animate_in_place !== undefined) payload.animate_in_place = options.animate_in_place;
            const response = await fetch(`${TRIPO_API_BASE}/task`, {
                method: "POST",
                headers: {
...
/**
 * Create an image-to-3D model task using Tripo AI
 * @param imageUrl URL or data URI of the image
 */
export async function createImageTo3DTask(imageUrl: string, options?: ImageTo3DOptions): Promise<string> {
    const keys = getAllTripoApiKeys();
    let lastError: any = null;
    ...
    for (let i = 0; i < keys.length; i++) {
        const apiKey = keys[i];
        try {
            console.log(`[Tripo Image-to-3D] Creating task with key ${i + 1}`);

            // First import the image
            const importPayload = {
                type: "upload_image",
                file: {
                    type: imageUrl.startsWith("data:") ? imageUrl.split(";")[0].split(":")[1].split("/")[1] : "png",
                    url: imageUrl
                }
            };
            
            // Note: Since Tripo uses a file upload task for image-to-3D, we need to adapt our API call based on documentation.
            // Using a standard payload assuming the API accepts a direct URL or requires an import_image task first.
            // For simplicity, we assume we can pass the URL directly as a file.

            const payload: Record<string, any> = {
                type: "image_to_model",
                file: {
                    type: imageUrl.startsWith("data:") ? "png" : imageUrl.split('.').pop() || "png",
                    url: imageUrl
                },
                model_version: options?.model_version || "v2.5-20250123",
            };
            ...
            const taskId = data.data.task_id;
            console.log(`[Tripo Image-to-3D] ✅ Task created: ${taskId}`);
            return taskId;

        } catch (error) {
            lastError = error;
            console.warn(`[Tripo Image-to-3D] Key ${i + 1} failed:`, error);
            if (!shouldTryNextKey(error)) throw error;
        }
    }
