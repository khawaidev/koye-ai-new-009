...
export function extractToolHints(text: string): string[] {
  ...
  const legacyMap: [RegExp, string][] = [
    ...
    [/<EDIT_FILE/g, "Editing file…"],
    [/<DELETE_FILE/g, "Deleting file…"],
    [/\[REPLACE_CODE:/g, "Applying code changes…"],
    [/\[CREATE_FOLDER:/g, "Creating folder…"],
    [/\[DELETE_FOLDER:/g, "Deleting folder…"],
    [/\[START_BACKGROUND_TASK:/g, "Starting background task…"],
    [/\[CHECK_TASK_STATUS:/g, "Checking task status…"],
  ]
...
export function stripToolMarkersForDisplay(text: string): string {
  ...
  const markerStarts = ["[TOOL_CALL:", "[CREATE_FILE:", "[EDIT_FILE:", "[DELETE_FILE:", "[EDIT_IMAGE:", "[START_BACKGROUND_TASK:", "[CHECK_TASK_STATUS:"]
...
function stripLegacyBracketMarkers(text: string): string {
  const markers = ["[CREATE_FILE:", "[EDIT_FILE:", "[DELETE_FILE:", "[EDIT_IMAGE:", "[START_BACKGROUND_TASK:", "[CHECK_TASK_STATUS:"]
  let result = text
...
export function parseToolCalls(content: string): ParseResult {
  ...
  // ─── 6. Parse LEGACY BACKGROUND TASKS markers ───
  const startTaskRegexes = [
    /\[START_BACKGROUND_TASK:\s*([^:]+?):\s*([^\]]+?)\]/gi,
  ]
  for (const regex of startTaskRegexes) {
    while ((match = regex.exec(content)) !== null) {
      toolCalls.push({
        id: uuidv4(),
  ...
  const checkTaskRegexes = [
    /\[CHECK_TASK_STATUS:\s*([^\]]+?)\]/gi,
  ]
  for (const regex of checkTaskRegexes) {
    while ((match = regex.exec(content)) !== null) {
      toolCalls.push({
        id: uuidv4(),
...
export function hasToolCalls(content: string): boolean {
  return (
    ...
    /\[REPLACE_CODE:/i.test(content) ||
    /\[CREATE_FOLDER:/i.test(content) ||
    /\[DELETE_FOLDER:/i.test(content) ||
    /<CREATE_FILE/i.test(content) ||
    /<EDIT_FILE/i.test(content) ||
    /<DELETE_FILE/i.test(content) ||
    /<EDIT_IMAGE/i.test(content) ||
    /\[START_BACKGROUND_TASK:/i.test(content) ||
    /\[CHECK_TASK_STATUS:/i.test(content)
  )
}
