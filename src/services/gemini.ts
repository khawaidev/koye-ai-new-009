...
import { withSmartRoute, withSmartRouteStream, type SmartRouteOptions, type SmartRouteResult } from "./geminiSmartRouter"

// Get API keys with fallback support
const GEMINI_API_KEYS = getApiKeys("VITE_GEMINI_API_KEY")

export interface GeminiRequestOptions extends SmartRouteOptions {
  thinkingLevel?: "low" | "medium" | "high"
}

function normalizeGeminiOptions(options?: string | GeminiRequestOptions): GeminiRequestOptions {
  if (!options) return {}
...
function buildGenerationConfig(thinkingLevel?: GeminiRequestOptions["thinkingLevel"]) {
...
8. BACKGROUND TASKS:
   - When a user asks you to do something that can run in the background concurrently (e.g. "generate an image in the background", "start a video generation while we talk"), use the [START_BACKGROUND_TASK: <type>: <prompt>] format.
   - To trigger it, output EXACTLY: [START_BACKGROUND_TASK: <type>: <prompt>]
   - Valid types are: image-generation, video-generation, 3d-model, audio, sprites, animations
   - Example: [START_BACKGROUND_TASK: image-generation: A cyberpunk city at night]
...
export async function sendMessageToGemini(
  ...
  options?: string | GeminiRequestOptions
): Promise<string> {
  ...
  const requestOptions = normalizeGeminiOptions(options)

  try {
    return await withSmartRoute(async (route: SmartRouteResult) => {
      const isFlashLive = route.modelId === "gemini-3-flash-live"
      
      const contents = messages.map(msg => ({
        role: msg.role === "model" ? "model" : "user",
        parts: msg.parts
          .filter(part => {
            if (isFlashLive && part.inlineData) return false
            return true
          })
          .map(part => {
            if (part.inlineData) {
      ...
      const config = requestOptions.thinkingLevel
        ? buildGenerationConfig(requestOptions.thinkingLevel)
        : (() => {
            const supportsThinking = route.modelId.includes("-3") || route.modelId.includes("thinking")
            const fallbackConfig: any = { temperature: 0.8 }
            if (supportsThinking) {
              fallbackConfig.thinkingConfig = { thinkingLevel: ThinkingLevel.HIGH }
            }
            if (isFlashLive) {
              fallbackConfig.responseMimeType = "text/plain"
            }
    ...
    }, estTokens, requestOptions)
  } catch (error) {
    if (error instanceof Error && error.message === "ALL_GEMINI_KEYS_EXHAUSTED") {
      console.warn("[Gemini] All Gemini routes exhausted")
      throw new Error("All Gemini models are currently rate limited or exhausted. Please try again shortly.")
    }
    throw error
  }
}
  ...
  try {
    const gen = withSmartRouteStream(async function* (route: SmartRouteResult) {
      ...
      const contents = messages.map(msg => ({
        ...
        parts: msg.parts
          .filter(part => {
            // If it's gemini-3-flash-live, we strictly use text input as requested
            if (isFlashLive && part.inlineData) return false
            return true
          })
          .map(part => {
      ...
      const config = requestOptions.thinkingLevel
        ? buildGenerationConfig(requestOptions.thinkingLevel)
        : (() => {
            // Preserve the older default behavior when no orchestrator thinking level is supplied.
            const supportsThinking = (
              route.modelId.includes("-3.1-") ||
              route.modelId.includes("-3-flash") ||
              route.modelId.includes("-3.5-flash")
            ) && !route.modelId.includes("lite")
            const fallbackConfig: any = { temperature: 0.8 }
            if (supportsThinking) {
              fallbackConfig.thinkingConfig = { thinkingLevel: ThinkingLevel.HIGH }
            }
            // For gemini-3-flash-live, ensure text output as requested
            if (isFlashLive) {
              fallbackConfig.responseMimeType = "text/plain"
            }
            return fallbackConfig
    ...
    if (error instanceof Error && error.message === "ALL_GEMINI_KEYS_EXHAUSTED") {
      console.warn("[Gemini] All Gemini stream routes exhausted")
      throw new Error("All Gemini models are currently rate limited or exhausted. Please try again shortly.")
    }
    throw error
  }
...
export async function sendMessageToGeminiWithThinking(
  ...
  try {
    return await withSmartRoute(async (route: SmartRouteResult) => {
      ...
      const config = buildGenerationConfig(requestOptions.thinkingLevel || "high")
