...
interface AppState {
  ...
  selectedModelId: string // The specific model ID selected when mode is 'models'
  ...
  setSelectedModelMode: (mode: ModelMode) => void
...
export const useAppStore = create<AppState>((set, get) => {
  ...
  const appStoreChannel = typeof window !== 'undefined' ? new BroadcastChannel('app-store-sync') : null
  let isAppStoreSyncing = false

  if (appStoreChannel) {
    appStoreChannel.onmessage = (event) => {
      if (event.data?.type === 'SYNC_GENERATED_FILES') {
        isAppStoreSyncing = true
        set({ generatedFiles: event.data.payload })
        isAppStoreSyncing = false
      }
    }
  }
  ...
  return {
    ...
    createNewSession: () => {
      ...
      // Check if an empty session already exists to prevent duplicates
      const existingEmptySession = state.sessions.find(s => s.messages.length === 0)
      if (existingEmptySession) {
        state.switchSession(existingEmptySession.id)
    ...
    setSelectedModelMode: (selectedModelMode) => set({ selectedModelMode }),
    ...
    addGeneratedFile: (path, content) => {
      ...
      if (!isAppStoreSyncing && appStoreChannel) {
        appStoreChannel.postMessage({ type: 'SYNC_GENERATED_FILES', payload: get().generatedFiles })
      }
    },
    setGeneratedFiles: (generatedFiles) => {
      set({ generatedFiles })
      if (!isAppStoreSyncing && appStoreChannel) {
        appStoreChannel.postMessage({ type: 'SYNC_GENERATED_FILES', payload: generatedFiles })
      }
    },
