...
import { saveVFSFile, deleteVFSFile, renameVFSFile, moveVFSFiles } from "../../services/vfs"
...
export function BuilderSidebar({ selectedFile, onSelectFile, onFileCreated, projectId, userId, onFileSynced, onUploadStateChange }: BuilderSidebarProps) {
    ...
    const { generatedFiles: rawGeneratedFiles, images, currentModel, addGeneratedFile, setGeneratedFiles, githubConnection: _githubConnection } = useAppStore()
    ...
    const handleFiles = async (files: File[]) => {
        ...
        try {
            ...
            for (let i = 0; i < readFiles.length; i++) {
                ...
                try {
                    saveVFSFile(projectId, finalPath, content).catch(e => console.warn('[VFS] upload sync failed:', e));
                    await saveSingleProjectFile(projectId, userId, '', finalPath, content, _githubConnection);

                    // Success — mark as synced before adding to local state to avoid "unsaved changes" flicker
    ...
    const handleCreateFile = () => {
        ...
        // Sync to VFS + R2 immediately (manual user action)
        if (projectId && userId) {
            saveVFSFile(projectId, filePath, content).catch(e => console.warn(`[VFS] create sync failed:`, e))
    ...
    const handleCreateFolder = () => {
        ...
        // Sync to VFS + R2 immediately (manual user action)
        if (projectId && userId) {
            saveVFSFile(projectId, indexFilePath, '').catch(e => console.warn(`[VFS] folder sync failed:`, e))
        ...
        if (isFolder) {
            ...
            for (const oldPath of Object.keys(updatedFiles)) {
                if (oldPath.startsWith(prefix)) {
                    ...
                    if (projectId && userId) {
                        deleteVFSFile(projectId, oldPath).catch(() => {})
                        saveVFSFile(projectId, newPath, content).catch(() => {})
            ...
            // Sync to VFS + R2
            if (projectId && userId) {
                renameVFSFile(projectId, selectedFile, newPath).catch(e => console.warn('[VFS] rename failed:', e))
        ...
        if (isFolder) {
            ...
            for (const path of Object.keys(updatedFiles)) {
                if (path.startsWith(prefix)) {
                    ...
                    // Sync to VFS + R2 (fire-and-forget)
                    if (projectId && userId) {
            ...
            delete updatedFiles[selectedFile]
            if (projectId && userId) {
                deleteVFSFile(projectId, selectedFile).catch(() => {})
                deleteProjectFile(projectId, userId, selectedFile, _githubConnection)
    ...
    const handlePaste = () => {
        ...
        if (isFolderPaste) {
            ...
            if (clipboard.operation === "cut") {
                ...
                for (const p of Object.keys(updatedFiles)) {
                    if (p.startsWith(prefix) && !p.startsWith(newFolderPath + '/')) {
                        ...
                        if (projectId && userId) {
                            deleteVFSFile(projectId, p).catch(() => {})
            ...
            if (projectId && userId) {
                saveVFSFile(projectId, newPath, clipboard.content).catch(() => {})
            ...
            if (clipboard.operation === "cut") {
                ...
                if (projectId && userId) {
                    deleteVFSFile(projectId, clipboard.path).catch(() => {})
    ...
    return (
        <div className="w-64 border-r border-border/60 bg-background flex flex-col h-full font-mono dark:bg-[#1f1f1e]">


            {/* Creation Buttons */}
            <div className="p-3 border-b border-border/60 flex gap-1.5 bg-background items-center dark:bg-[#1f1f1e]">
                <input 
                    type="file" 
                    multiple 
                    className="hidden" 
                ...
                <Button
                    ...
                    className="flex-1 px-1 border border-border hover:bg-foreground hover:text-background bg-background text-foreground font-mono text-[10px] font-bold transition-all shadow-sm"
                    title="New File"
                >
                ...
                <div className="relative" ref={settingsMenuRef}>
                    ...
                    {showSettingsMenu && (() => {
                        ...
                        return (
                        <div className="absolute right-0 top-full mt-1 w-40 bg-background border-2 border-border shadow-[4px_4px_0px_0px_currentColor] z-50 rounded-md overflow-hidden">
                            <button
            ...
            <div
                ref={dropZoneRef}
                className={cn(
                    "flex-1 overflow-y-auto py-2 bg-background relative transition-colors dark:bg-[#1f1f1e]",
            ...
            >
                ...
                {fileTree.length === 0 ? (
                    <div className="px-4 py-8 text-center text-muted-foreground text-xs font-mono">
