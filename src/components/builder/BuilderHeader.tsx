import { Play, Redo, Undo, Save } from "lucide-react"
...
export function BuilderHeader({ projectName, onUndo, onRedo, onPlay, onSave, canUndo, canRedo, hasUnsavedChanges }: BuilderHeaderProps) {
        ...
        <div className="h-14 border-b-2 border-border bg-background flex items-center justify-between px-6 shrink-0 font-mono text-foreground">
            ...
            <div className="flex items-center gap-4">
                {onSave && hasUnsavedChanges && (
                    <Button
                        onClick={onSave}
                        className="bg-green-500 text-white border-green-600 hover:bg-green-600 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,0.3)] hover:translate-x-[2px] hover:translate-y-[2px] font-mono text-xs font-bold flex items-center gap-2 px-4 py-2 border-2 transition-all rounded-none"
                    >
